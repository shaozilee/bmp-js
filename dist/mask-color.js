...
function maskColor(maskRed, maskGreen, maskBlue, maskAlpha) {
    var maskRedR = (~maskRed + 1) & maskRed;
    var maskGreenR = (~maskGreen + 1) & maskGreen;
    var maskBlueR = (~maskBlue + 1) & maskBlue;
    var maskAlphaR = (~maskAlpha + 1) & maskAlpha;
    var shiftedMaskRedL = maskRed / maskRedR + 1;
    var shiftedMaskGreenL = maskGreen / maskGreenR + 1;
    var shiftedMaskBlueL = maskBlue / maskBlueR + 1;
    var shiftedMaskAlphaL = maskAlpha / maskAlphaR + 1;
    return {
        shiftRed: function (x) {
            return (((x & maskRed) / maskRedR) * 0x100) / shiftedMaskRedL;
        },
        shiftGreen: function (x) {
            return (((x & maskGreen) / maskGreenR) * 0x100) / shiftedMaskGreenL;
        },
        shiftBlue: function (x) {
            return (((x & maskBlue) / maskBlueR) * 0x100) / shiftedMaskBlueL;
        },
        shiftAlpha: maskAlpha !== 0
            ? function (x) {
                return (((x & maskAlpha) / maskAlphaR) * 0x100) / shiftedMaskAlphaL;
            }
            : function () { return 255; }
    };
}
