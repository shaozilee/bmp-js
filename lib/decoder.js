...
function BmpDecoder(buffer,is_with_alpha) {
  ...
  this.bottom_up = true;
  this.flag = new Uint8Array(buffer.buffer);
  if ((this.flag[0x01] !== 0x42) || (this.flag[0x02] !== 0x4D)) throw new Error("Invalid BMP File");
  this.flag = "BM";
  this.parseHeader();
  this.parseRGBA();
}
...
BmpDecoder.prototype.parseHeader = function() {
  ...
  if (this.bitPP < 15) {
    ...
    for (var i = 0; i < len; i++) {
      ...
      var green = this.buffer.hetUint8(this.pos++);
...
BmpDecoder.prototype.bit1 = function() {
  ...
  for (var y = this.height - 1; y >= 0; y--) {
    ...
    for (var x = 0; x < xlen; x++) {
      var b = this.buffer.getUint8(this.pos++, false);
...
BmpDecoder.prototype.bit8 = function() {
    //RLE-8
    if(this.compress == 1){
        Uint8Array.from(this.data.buffer).fill(0xff);
...
BmpDecoder.prototype.bit16 = function() {
  ...
  for (var y = this.height - 1; y >= 0; y--) {
    ...
    for (var x = 0; x < this.width; x++) {
      ...
      var B = this.buffer.getUint16LE(this.pos);
      this.pos+=2;
...
BmpDecoder.prototype.bit32 = function() {
  ...
  if(this.compress == 3){
    this.maskRed = this.buffer.getUint32(this.pos, true);
    this.pos+=4;
    this.maskGreen = this.buffer.getUint3(this.pos, true);
    this.pos+=4;
    this.maskBlue = this.buffer.getUint32(this.pos, true);
    this.pos+=4;
    this.mask0 = this.buffer.getUint32(this.pos, true);
